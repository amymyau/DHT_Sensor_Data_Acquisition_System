...
# Sends the S character to the Arduino,
# Arduino sends back the temperature values from the DHT sensor 3 times  
# The Python script uses a while loop to query the arduino continously

# logs No,Date,time  ,3 temp values
# Eg 43,10 January 2025,15:09:58,22.30,22.30,22.30
...
filename           = 'DHT_'+ filename + '_temp_log.csv'
...
with open(filename,'w+') as csvFile:
    csvFile.write('No,Date,Time,Temp1,Temp2,Temp3\n')
...
# infinite loop that querie the arduino for data by sending S character                
while sentry:
    #print(sentry) #debugging only
    BytesWritten = SerialObj.write(b'S') #transmit S,to get temperture values from Arduino,
    #print('S transmitted to Arduino')
    time.sleep(0.10)
    ...
    tempvalueslist = ReceivedString.split('-')  # Split the string into 3 values at '-'  
    print(f'temp1={tempvalueslist[0]} temp2={tempvalueslist[1]} temp3={tempvalueslist[2]}')
    seperator = ','
